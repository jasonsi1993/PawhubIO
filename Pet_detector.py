...
# Set up camera constants (Using 720P)
IM_WIDTH = 1280
IM_HEIGHT = 720

# Select camera type (if user enters --usbcam when calling this script,
# a USB webcam will be used)
camera_type = 'picamera' #Using Raspery for development
parser = argparse.ArgumentParser() #Reference https://docs.python.org/3/library/argparse.html
...
PATH_TO_CKPT = os.path.join(CWD_PATH,MODEL_NAME,'frozen_inference_graph.pb') #Reference: https://stackoverflow.com/questions/52934795/what-is-difference-frozen-inference-graph-pb-and-saved-model-pb
...
frame_inside = (IM_WIDTH,IM_HEIGHT)
...
def pet_detector(frame):
    ...
    cv2.putText(frame,"Cat Detected",(frame_inside[0]+10,frame_inside[1]-10),font,1,(20,255,255),3,cv2.LINE_AA)
    ...
    if (((int(classes[0][0]) == 17) or (int(classes[0][0] == 18) or (int(classes[0][0]) == 88))) and (pause == 0)):
      ...
      #  if ((x > 0) and (x < frame_inside[0]) and (y > 0 and (y < frame_inside[1])):
        #    detected_cat = inside_counter + 1
